...
- Repo root: this directory. Fork of Helium314/HeliBoard; sole remote `upstream` = Helium314/HeliBoard (public, read-only). There is no `origin` and no GitHub credentials in this account: you never push or pull from GitHub. The human mirrors this repo from another account, reviews there, and handles all GitHub traffic. Work on branch `nextword`.
...
4. One fix per commit, small commits, imperative messages. Never run `git push`. Never rewrite history on `nextword` — no rebase, no `git commit --amend`, no reset of committed work. Treat every commit as published the moment it is made, because the human's mirror fetches your commits as-is; if a commit was wrong, fix it with a new commit.
